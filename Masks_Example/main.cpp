//masks
//ways we can highlight or shadow a bit in a byte
//doing a set of operations in a unified manner

//set a bit position    &=(~mask)
//reset bit positioin   &(AND)
//check a bit position  & (AND)
//toggle a bit position aka change it  ^(mask) or XOR
//we use OR (|=) to change the bit of interest to opposite bit. if 1, changes to 0 and vise versa
